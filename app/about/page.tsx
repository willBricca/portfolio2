...
export default function AboutPage() {
  ...
  const unlockScroll = useCallback(() => {
    ...
    window.scrollTo(0, y);
    delete (document.body as HTMLBodyElement & { dataset: { lockY?: string } })
      .dataset.lockY;
  }, []);
  ...
  // wheel / keyboard / touch listeners for hero <-> white transition
  useEffect(() => {
    let touchStartY = 0;
    const atTop = () =>
    ...
    const onTouchMove = (e: TouchEvent) => {
      ...
      if (!onWhite && atTop() && dy > 12) {
        fadeToWhite();
      } else if (onWhite && atTop() && dy < -12) {
        fadeToHero();
      }
    };
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      const cardWidth = container.scrollWidth / 5; // 5 cards total (even though we currently render 3)
  ...
  const scrollToCard = (index: number) => {
    const container = scrollContainerRef.current;
    if (!container) return;
    const cardWidth = container.scrollWidth / 5;
    container.scrollTo({
  ...
  return (
    <>
      ...
      <style jsx global>{`
        ...
        /* 2) Kill SiteNav's full-bleed transform so it doesn't jitter */
      ...
      <main className="relative z-0">
        {/* spacer so hero fills first screen when we're in hero mode */}
        <section
          className={!onWhite ? "h-[100svh]" : "h-0"}
          aria-label="Hero spacer"
        />
        ...
        >
          <div className="mx-auto max-w-6xl px-4 md:px-8">
            <div className="mt-4 text-zinc-700 md:text-lg leading-relaxed max-w-3xl space-y-4">
              <p>
                ... and I truly love the iterative process of building things,
                and watching them fail. I've failed more in the past two years
                than in my entire life before that, and it's been an incredible
                time of growth.
              </p>
              <p>
                Starting out with Mechanical Engineering, it took time to find
                my passion within it. I've quickly realized over the second half
                of my degree that watching autonomous systems interact with the
                world based on simple first principles is special.
              </p>
              <p>
                Beyond engineering, I enjoy producing and analyzing movie
                soundtracks. Songs from Tron, Interstellar, and The Lion King:
                all incredible backdrops for building. I love the clarity that
                endurance sports bring, and the humbling feeling of being
                outdoors. Most of all, I love my family and friends. There's
                nothing better than building with or simply living alongside
                good, curious people.
              </p>
            </div>
          </div>
        ...
        >
          <div className="mx-auto max-w-7xl px-4 md:px-8 py-16 w-full">
            ...
            <div className="relative">
              ...
              >
                <article
                  className="rounded-2xl overflow-hidden border border-zinc-200/80 bg-white shadow-sm flex-shrink-0 snap-center"
                ...
                {[0, 1, 2].map((index) => (
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: "4px",
                      backgroundColor:
                        currentCard === index ? "#18181b" : "#d4d4d8",
                      border: "none",
                      cursor: "pointer",
                    }}
                  />
